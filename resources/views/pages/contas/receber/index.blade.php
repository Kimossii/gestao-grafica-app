...
@section('content')
    <div class="max-w-7xl mx-auto py-6">
        <div class="bg-white p-6 rounded shadow">
            <h2 class="text-2xl font-bold mb-4">Lista de Contas a Receber</h2>
            @if ($errors->any())
                    ...
                    <table class="min-w-full border text-sm">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-2 border">Data Recebimento</th>
                        ...
                        <tbody>
                            @foreach ($contas as $conta)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2 border">{{ $conta->cliente->nome ?? 'N/A' }}</td>
                                    <td class="px-4 py-2 border">{{ $conta->observacoes }}</td>
                                    <td class="px-4 py-2 border">{{ ucfirst($conta->status) }}</td>
                                    <td class="px-4 py-2 border text-center">
                                        <a href="{{ route('contas.editar.receber', $conta->id) }}"
                                            class="text-blue-600 hover:underline">Editar</a>
                                        <span class="mx-2">|</span>
                                        <form action="{{ route('contas.excluir.receber', $conta->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
